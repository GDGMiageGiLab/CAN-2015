...
public class SocialFetchService extends Service {
    ...
    private class DownloadTwitterTask extends AsyncTask<String, Void, String> {
        ...
        public void doneFetchingAll() {
            com.gdg.miagegi.can2015.utils.Log.d(LOG_TAG, "Done Fetching all");

            if (tweetscount > 0) {
                String message = "";
                if (tweetscount == 1) {
                    message = tweetscount + " nouveau tweet.";
                } else {
                    message = tweetscount + " nouveaux tweets.";
                }

                Intent notificationIntent = new Intent(SocialFetchService.this, MainActivity.class);

                notificationIntent.putExtra("position", 0).addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
                PendingIntent contentIntent = PendingIntent.getActivity(SocialFetchService.this, 0,
